...
import { Market, Media, WXDAI, Market__factory, Media__factory } from "../typechain";
...
async function main() {
  ...
  const wallet = new ethers.Wallet(`0x${process.env.PRIVATE_KEY_CONTRACT}`, provider);
  ...
  if (args.chainId === 31337) {
    ...
    const WxDai = (await ethers.getContractFactory(
      "WXDAI",
      wallet
    )) as WXDAI;
    ...
    console.log(
      `Wxdai deploying to ${tx.address}. Awaiting confirmation...`
    );
    await tx.deployed();
    console.log(
      `Wxdai deployed!`
    ...
    if (!args.wxdai) {
      throw new Error("--wxdai address to deployed native coin wrapper contract is required");
    }
  ...
  console.log(
    `Market deploying to ${tx.address}. Awaiting confirmation...`
  );
  ...
  txReceipt = await provider.getTransactionReceipt(market.deployTransaction.hash);
  console.log('Market deploy gas usage: ', txReceipt.gasUsed.toNumber());

  // We get the contract to deploy
  const Media = (await ethers.getContractFactory(
    "Media",
    wallet
  )) as Media;
  ...
  console.log(
    `Media deploying to ${tx.address}. Awaiting confirmation...`
  );
  ...
  txReceipt = await provider.getTransactionReceipt(media.deployTransaction.hash);
